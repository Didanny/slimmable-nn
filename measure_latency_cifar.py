...
import models
import data
from utils import Flags, Profile
...
@torch.no_grad()        
def run_model(model, input, iterations):
...
def main(opt: argparse.Namespace):
    ...
    train_loader, _ = getattr(data, opt.dataset)()
    _, val_loader = getattr(data, opt.dataset)(batch_size=opt.batch_size)
    
    # Get the criterion
    criterion = nn.CrossEntropyLoss()
    
    # Set up the dummy input
    dummy_input = torch.rand((opt.batch_size, 3, 32, 32))
    dummy_input = dummy_input.to(device, non_blocking=True)
    ...
    for i, model_name in enumerate(opt.models):
        ...
        # Warmup if cuda
        if torch.cuda.is_available():
            run_model(model, dummy_input, 10)
        
        # Measure latencies for each width
        for width_mult in sorted(FLAGS.width_mult_list, reverse=True):
            model.apply(lambda m: setattr(m, 'width_mult', width_mult))
            with Profile() as p:
                run_model(model, dummy_input, 100)    
                
            val_meters[f'{width_mult}']['latency'] = p.t
        
        # flush_meters(val_meters)
        results[i] += [val_meters[str(width)]['latency'] for width in sorted(FLAGS.width_mult_list, reverse=True)]
        print(results[i])
